...
func (reepositorio Usuarios) AtualizarSenha(usuarioID uint64, senha string) error {
	statement, erro := reepositorio.db.Prepare("update usuarios set senha = ? where id = ?")
	if erro != nil {
		return erro
	}
	defer statement.Close()
